...
const callApi = (endpoint, schema, query = null, isDownload = false) => {
    ...
    const {method = 'get', data , headers = {},withCredentials=true,isServer=false} = query || {}

    // 根据isServer判断是服务端渲染还是客户端渲染
    if(isServer){
      API_ROOT = 'http://'+apiConfig.apiDomain
    }else{
      fullUrl = `/api${fullUrl}` //代理到node服务器
    }
